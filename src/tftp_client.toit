...
class TFTPClient:
  ...
  buffered /reader.BufferedReader? := null
  ...
  write-bytes data/ByteArray --filename -> Result:
    reader := bytes.Reader data
    return write-stream reader --filename=filename

  write-stream  astream /reader.Reader --.filename /string  -> Result:
    mode = OCTET
    buffered = reader.BufferedReader astream
    return write_
  ...
  can-ensure_ size/int -> bool:
    return buffered.can-ensure size

  read-bytes_ size/int -> ByteArray:
    return buffered.read-bytes size
...
class ClientExchange:
  ...
  */
  read-with client /TFTPClient -> none:  // Reads are initiated from the client.
    while opcode != EXIT:
  ...
  writer-handle received-bytes/ByteArray -> none:
    ...
    else if opcode == WRQ   and received.opcode == TIMEOUT: resend-WRQ
    else if opcode == DATA  and received.opcode == ACK:     keep-writing (received as PacketACK)
    else if opcode == DATA  and received.opcode == TIMEOUT: resend-write
  ...
  exit-error received /PacketERROR -> none:
    opcode = EXIT
    result = Result.fail "Server error while sending data: $received.error-msg"

  writer-bytes -> ByteArray:
    if tries > 0: return cached
    if opcode == WRQ: return wrq-frame
    if opcode == DATA: return next-data-frame
